...
fruttare operatori più complessi come il fattoriale e l'elevamento a potenza.
...
Data un'instanza di tipo strutturato, è possibile accedere ai campi utilizzando l'operatore *"."*.
...
La libreria previene l'utente dall'eventuale ridefinizione delle funzioni elencate, ed è definita separatamente dalla
grammatica di JPocl.
...
Non è possibile effettuare overloading delle funzioni definite, nè ricorrere alla ricorsione mutua, mentre è possibile
...
- il comando *echo* per la stampa su terminale, è utilizzabile sia per i tipi di dato primitivi che per le istanze di tipi strutturati;
...
- nuova dichiarazione di una funzione già dichiarata nel codice;
- nuova dichiarazione di un tipo strutturato già dichiarato nel codice;
...
La variabile intera a, anche se dichiarata nello scope top-level, non è visibile all'interno di f(Z). In maniera simile,
il parametro a non è visibile all'esterno.
...
Come si può osservare, Jasmin mette a disposizione un linguaggio leggermente più astratto rispetto al bytecode puro di Java,
non è presente ad esempio la gestione della tabella delle costanti, mantenendo però le medesime istruzioni mnemoniche della JVM.
...
###Esecuzione
JPocl è eseguibile sia in Windows che in Linux; data la necessità di eseguire il comando "java" viene richiesto
...
L'eseguibile è all'interno della cartella Resources.
